...
namespace parec {
	...
	>
	std::function<std::future<O>(I)> parec(const rec_defs<Defs...>& );
	...
	namespace detail {
		...
		struct caller {
			...
			std::future<O> call(const F& f, const I& i, const D& d, const Args& ... args) const {
		...
		struct caller<0> {
			template<typename O, typename F, typename I, typename D, typename ... Args>
			std::future<O> call(const F& f, const I& i, const D& d, const Args& ... args) const {
				return std::async(f,i,parec<0>(d),args...);
			}
		};
	...
	struct rec_defs : public std::tuple<Defs...> {
		...
		template<
			...
			typename I
		>
		std::future<O> call(const I& in) const {
	...
	template<
		...
		typename O = typename type_at<i,type_list<Defs...>>::type::out_type
	>
	std::function<std::future<O>(I)> parec(const rec_defs<Defs...>& defs) {
		auto x = std::get<i>(defs);
		return [=](const I& in)->std::future<O> {
